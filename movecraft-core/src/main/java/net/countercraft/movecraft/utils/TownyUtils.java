...
public final class TownyUtils {
        ...
        if (t != null) {
            try {
                ...
                Movecraft.getInstance().getLogger()
                         .log(Level.SEVERE, "Town Spawn of " + t.getName() + " wasn't found!", ex);
                //town hasn't spawn
            }
        }
        return null;
        ...
        if (!worldsMap.containsKey(wName)) {
            ...
            if (tw != null) {
                if (tw.isUsingTowny()) {
                    ...
                    Movecraft.getInstance().getLogger()
                             .log(Level.INFO, "Added default Towny settings for world {0} to the config file.",
                                  w.getName());
                    oNew = true;
                } else {
                    return null;
    ...
    private static void saveWorldLimits() {
        ...
        for (Map.Entry<String, TownyWorldHeightLimits> entry : worldsMap.entrySet()) {
            World w = Movecraft.getInstance().getServer().getWorld(entry.getKey());
            if (w != null) {
                TownyWorld tw = getTownyWorld(w);
                if (tw != null) {
                    if (tw.isUsingTowny()) {
                        Map<String, Integer> townyTWorldMap = new HashMap<>();
                        TownyWorldHeightLimits twhl = entry.getValue();
                        ...
                        townyWorldsMap.put(entry.getKey(), townyTWorldMap);
                        Movecraft.getInstance().getConfig().set(TOWN_HEIGHT_LIMITS + "." +
                                                                entry.getKey(), townyTWorldMap);
                    }
                }
            }
        }
    ...
    private static void loadTownyConfig() {
        ...
        if (csObj != null) {
            ...
            for (String worldName : worlds) {
                ...
                twhl.world_min = fc.getInt(TOWN_HEIGHT_LIMITS + "." + worldName + "." + TOWN_MIN,
                                           TownyWorldHeightLimits.DEFAULT_WORLD_MIN);
                twhl.world_max = fc.getInt(TOWN_HEIGHT_LIMITS + "." + worldName + "." + TOWN_MAX,
                                           TownyWorldHeightLimits.DEFAULT_WORLD_MAX);
                twhl.above_town = fc.getInt(TOWN_HEIGHT_LIMITS + "." + worldName + "." + TOWN_ABOVE,
                                            TownyWorldHeightLimits.DEFAULT_TOWN_ABOVE);
                twhl.under_town = fc.getInt(TOWN_HEIGHT_LIMITS + "." + worldName + "." + TOWN_UNDER,
                                            TownyWorldHeightLimits.DEFAULT_TOWN_UNDER);
