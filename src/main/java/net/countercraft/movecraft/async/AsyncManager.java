...
public class AsyncManager extends BukkitRunnable {
    ...
    private final HashMap<org.bukkit.entity.TNTPrimed, Double> TNTTracking = new HashMap<org.bukkit.entity.TNTPrimed,
            Double>();
    private final HashMap<Craft, HashMap<Craft, Long>> recentContactTracking = new HashMap<Craft, HashMap<Craft,
            Long>>();
    private HashMap<org.bukkit.entity.SmallFireball, Long> FireballTracking = new HashMap<org.bukkit.entity
            .SmallFireball, Long>();
    ...
    private boolean isRegionBlockedPVP(MovecraftLocation loc, World w) {
        ...
        if (Settings.WorldGuardBlockSinkOnPVPPerm == false) return false;
                    ...
                    if (pcraft != null && pcraft.getSinking() == false) {
                        if (pcraft.getType().getSinkPercent() != 0.0 && pcraft.isNotProcessing()) {
                            ...
                            if (ticksElapsed > Settings.SinkCheckTicks) {
                                ...
                                HashMap<ArrayList<Integer>, Integer> foundFlyBlocks = new HashMap<ArrayList<Integer>,
                                        Integer>();
                                ...
                                if (isSinking &&
                                    ...
                                    if (isSinking && pcraft.isNotProcessing()) {
                                        Player p = CraftManager.getInstance().getPlayerFromCraft(pcraft);
                                        Player notifyP = pcraft.getNotificationPlayer();
                                        if (notifyP != null) notifyP.sendMessage(String.format(
